...
searchHidden: "false"
---
Today I'm going to set up my local weather station in my yard that will feed data to my database that stores the forecast data I'm pulling from NWS.  
...
I started by saving the raw POSTs from the Ecowitt console as a JSON file to see how the data was being sent.  I found that at this point, with the data from the weather station sensor array and the indoor temperature and humidity sensor, all the data is just one giant JSON structure without any nesting.  I don't want to save all of the data to a single table in the database, so in my script, I defined which fields belonged to which sensor.  

After divvying the data up into smaller chunks, I defined my table schemas and created them in my TimescaleDB instance.  With that complete, all I needed to do was steal the database module from the NWS scraper and modify the insert statements to fit this data and the respective table schemas.
